...
# Origin: https://github.com/taolei87/sru

# Modification:
#   - enable parameter adaptation depending on the value of interpretation neuron
#   in the linear transformation layer of SRU, here in the SRUCell
...
class SRUCell(nn.Module):
    def __init__(self, n_in, n_out, dropout=0, rnn_dropout=0, bidirectional=False,
            use_tanh=1, use_relu=0, use_selu=0, weight_norm=False, layer_norm=False, highway_bias=0, index=-1, n_actions=0, func='add'):
        ...
        if self.n_actions > 0:
            if 'transf' in func:
                self.wa = nn.Parameter(torch.Tensor(self.n_actions, n_in, self.size_per_dir*2 if bidirectional else self.size_per_dir))
            elif 'attn_eh' in func:
                self.wa_h = nn.Parameter(torch.Tensor(self.n_actions, n_in, 1))
                self.wa_e = nn.Parameter(torch.Tensor(self.n_actions, 1, self.size_per_dir*2 if bidirectional else self.size_per_dir))
                def attn(w1, wh, we):
                    # w1 [emb x 3*(2hid)]
                    # wh [emb x 1]
                    # we [1 x 3*(2hid)]
                ...
                self.wa = nn.Parameter(torch.Tensor(self.n_actions, n_in, 1))
                def attn(w1, w2):
                    # w1 [emb x 3*(2hid)]
                    # w2 [emb x 1]
                    a = torch.mul(w1, w2)
                    score = F.softmax(a.sum(0), dim=-1).unsqueeze(0)# [1 x 3*(2hid)]
                    wr = torch.mul(w1, score)
                    return wr
                self.func = lambda a,b: attn(a,b) 

            elif 'attn_e' in func:
                self.wa = nn.Parameter(torch.Tensor(self.n_actions, 1, self.size_per_dir*2 if bidirectional else self.size_per_dir))
                def attn(w1, w2):
                    # w1 [emb x 3*(2hid)]
                    # w2 [1 x 3*(2hid)]
                    a = torch.mul(w1, w2)
                    score = F.softmax(a.sum(1), dim=-1).unsqueeze(1)  # [emb x 1] 
                    wr = torch.mul(w1, score)
                    return wr
                self.func = lambda a,b: attn(a,b)
                ...
                def conv_forw(w, a):
                    ...
                    out = w.view(self.n_in, -1, self.k).unsqueeze(0).transpose(0,3).squeeze().contiguous().unsqueeze(0)
                    for i in range(self.cnn_layers):
                        if i != self.cnn_layers-1:
                            out = F.relu(self.w_conv[i][a](out))
                        else:
                            out = self.w_conv[i][a](out)
                    # [1, k, emb, hid, 1] >> [1, emb, hid, k] >> [emb, hid*k]
                    out = out.unsqueeze(4).transpose(1,4).contiguous().view(self.n_in, -1)
                    return out

                def conv_forw_res(w, a):
                    # w1 [emb x k*hid]
                    # [emb, hid, k] >> [1, emb, hid, k] >> [k, emb, hid, 1] >> [1, k, emb, hid]
                    out = w.view(self.n_in, -1, self.k).unsqueeze(0).transpose(0,3).squeeze().contiguous().unsqueeze(0)
                    for i in range(self.cnn_layers):
                        if i != self.cnn_layers-1:
                            if i%2==0:
                                if i == 0:
                                    ...
                                    out = F.relu(self.w_conv[i][a](out) + res)
                                    res = out
                            else:
                                out = F.relu(self.w_conv[i][a](out))
                        else:
                            if self.cnn_layers == 3:
                                out = self.w_conv[i][a](out+res)
                            else:
                                out = self.w_conv[i][a](out)
                    # [1, k, emb, hid, 1] >> [1, emb, hid, k] >> [emb, hid*k]
                    out = out.unsqueeze(4).transpose(1,4).contiguous().view(self.n_in, -1)
                ...
                self.func = lambda a,b: conv_forw(a,b)
                ...
                self.wa = nn.Parameter(torch.Tensor(2, self.n_actions,
                out_size, out_size
                ))
                self.func = lambda a,w: torch.mul(a,F.sigmoid(torch.mm(a,w)))
            else:
                self.wa = nn.Parameter(torch.Tensor(self.n_actions,
                ...
                if 'mul' in func:
                    ...
                    self.func = lambda a,b: torch.add(a,b)
                elif 'mul_s2' in func:
                    self.func = lambda a,b: torch.mul(a,F.sigmoid(b)) + torch.mul(b,1-F.sigmoid(b)) 
    ...
    def init_weight(self, rescale=True):
        ...
        if self.n_actions>0:
            if self.n_func == 'g_hc':
                self.wa.data.uniform_(-val_range, val_range)
                w = self.wa.data.view(2*self.n_out, -1, 2*self.n_out, 1)
                if self.dropout>0:
                    w[:,:,:,0].mul_((1-self.dropout)**0.5)
                if self.rnn_dropout>0:
                    w.mul_((1-self.rnn_dropout)**0.5)
            elif 'attn' in self.n_func:
                if 'attn_eh' in self.n_func:
                    val_range = (3.0/self.out_size)**0.5
                    self.wa_e.data.uniform_(-val_range, val_range)
                    w = self.wa_e.data.view(1, -1, self.n_out, self.k)
                    if self.dropout>0:
                        w[:,:,:,0].mul_((1-self.dropout)**0.5)
                    if self.rnn_dropout>0:
                        w.mul_((1-self.rnn_dropout)**0.5)
                    ...
                    if self.k == 4:
                        w[:,:,:,3].mul_(self.scale_x)
                    val_range = (3.0/self.n_in)**0.5
                    self.wa_h.data.uniform_(-val_range, val_range)
                    w = self.wa_h.data
                    if self.dropout>0:
                        w.mul_((1-self.dropout)**0.5)

                elif 'attn_e' in self.n_func:

                    val_range = (3.0/self.n_in)**0.5
                    self.wa.data.uniform_(-val_range, val_range)
                    w = self.wa.data.view(1, -1, self.n_out, self.k)
                    if self.dropout>0:
                        w[:,:,:,0].mul_((1-self.dropout)**0.5)
                    if self.rnn_dropout>0:
                        w.mul_((1-self.rnn_dropout)**0.5)
                    ...
                    if self.dropout>0:
                        w.mul_((1-self.dropout)**0.5)
            elif not 'conv' in self.n_func:
                self.wa.data.uniform_(-val_range, val_range)
                w = self.wa.data.view(self.n_in, -1, self.n_out, self.k)
                if self.dropout>0:
                    w[:,:,:,0].mul_((1-self.dropout)**0.5)
                if self.rnn_dropout>0:
                    w.mul_((1-self.rnn_dropout)**0.5)
    ...
    def forward(self, input, c0=None, actions=None, use_a=True):
        ...
        if self.n_actions>0 and self.n_func != 'g_hc' and use_a:
            length = x.size(0) if x.dim() == 3 else 1
            a_oh = one_hot(actions, self.n_actions).unsqueeze(2) # [batch x n_actions x 1]
            u = []
            xt_2d = x.transpose(0,1).contiguous().view(batch*length, n_in)
            for a in range(self.n_actions):
                if 'attn_eh' in self.n_func:
                    w_i = self.func(weight, self.wa_h[a], self.wa_e[a])
                elif 'conv' in self.n_func:
                    w_i = self.func(weight, a)
                elif 'transf' in self.n_func:
                    w_i = self.wa[a]
                else:
                    w_i = self.func(weight, self.wa[a])
                u_i = xt_2d.mm(w_i).view(batch, -1)
                u.append(u_i)
            u = torch.stack(u, 1) # [batch x actions x len*hid]
            u = torch.mul(u, a_oh).sum(1).view(batch, length, -1)
            u = u.transpose(0,1).contiguous().view(length*batch, self.weight.size(-1))
        else:
            u = x_2d.mm(weight)
        ...
        if self.training and (self.dropout>0):
            ...
            h, last_hidden = SRU_Compute(u, input, self.bias, c0, mask_h)
        else:
            h, last_hidden = SRU_Compute(u, input, self.bias, c0)

        if self.n_actions>0 and self.n_func == 'g_hc' and use_a:
            length = x.size(0) if x.dim() == 3 else 1
            a_oh = one_hot(actions, self.n_actions).unsqueeze(2) # [batch x n_actions x 1]
            uh, uc = [], []
            wh, wc = self.wa[0], self.wa[1]
            h_2d = h.transpose(0,1).contiguous().view(batch*length, self.out_size)
            for a in range(self.n_actions):
                h_i = self.func(h_2d, wh[a]).view(batch, -1)
                c_i = self.func(h_2d, wh[a]).view(batch, -1)
                uh.append(h_i); uc.append(c_i)

            uh = torch.stack(uh, 1) # [batch x n_actions x len*hid]
            h = torch.mul(uh, a_oh).sum(1)
            h = h.transpose(0,1).contiguous().view(length, batch, -1)
            uc = torch.stack(uc, 1) # [batch x n_actions x hid]
            last_hidden = torch.mul(uc, a_oh).sum(1)
